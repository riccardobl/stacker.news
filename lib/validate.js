...
export function territorySchema (args) {
  return object({
    ...
    nsfw: boolean(),
    requireBondToPost: boolean().required('required'),
    bondCostSats: intValidator.positive('must be positive').min(1, 'must be at least 1').max(100000, 'must be at most 100k'),
    bondDurationDays: intValidator.positive('must be positive').min(1, 'must be at least 1').max(365, 'must be at most 365')
  })
}
